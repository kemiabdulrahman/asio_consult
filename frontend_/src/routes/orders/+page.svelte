<script>
  ...
  onMount(async () => {
    ...
    const userToken = typeof localStorage !== 'undefined' ? localStorage.getItem('user_token') : null;
...
<div class="min-h-screen bg-gray-50 py-12">
  {#if notAllowed}
    <div class="max-w-xl mx-auto bg-white rounded-lg shadow-md p-8 mt-16 text-center">
      <h2 class="text-2xl font-bold mb-4">Not Authorized</h2>
      <p class="text-gray-600 mb-6">You must be logged in as a user or admin to view your orders.</p>
      <a href="/login" class="inline-block px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors">Login</a>
    </div>
  {:else}
                      ...
                      {#each order.items as item}
                        <div class="flex justify-between text-sm">
                          ...
                          <p class="text-gray-900 font-medium">
                            ₦{((item.price * (item.quantity || 1)) / 100).toLocaleString('en-NG', {
                ...
                <div class="flex justify-end items-center gap-8 pt-4 border-t border-gray-200">
                  <div>
                    ...
                    <p class="text-2xl font-bold text-primary-600">
                      ₦{(order.total / 100).toLocaleString('en-NG', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}
                    </p>
